...
ticketsController.updateTicketStatus = (req, res, next) => {
  ...
  const updateTicket = { //update snaps_given and feedback
    text: `
      UPDATE tickets
      SET status = $1, mentor_id = $2, snaps_given = $3, feedback = $4
      WHERE _id = $5;
    `,
    values: [status, mentorId, snaps_given, feedback, ticketId]
  };
  db.query(updateTicket)
    .then(success => next())
    .catch(err => {
      return next({
        log: "Error occured in ticketsController.updateTicket",
        status: 400,
        message: {
          err: err
        }
