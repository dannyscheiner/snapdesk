...
roomsController.addRooms = (req, res, next) => {
  ...
  >>> > fb843dda762ea8d9ada9cf7a8aef72d55505f462
  const addRoom = {
    text: `
      INSERT INTO rooms
      (name, admin_id)
...
roomsController.getRooms = (req, res, next) => {
  ...
  const getRooms = {
    text: "SELECT DISTINCT rooms._id as id, rooms.name, rooms.admin_id as admin FROM rooms INNER JOIN rooms_users ON rooms._id=rooms_users.room_id WHERE rooms_users.user_id =$1 AND rooms_users.banned = false",
    values: [userId]
  };
  db.query(getRooms)
    .then(getRooms => {
...
roomsController.getActiveRoom = (req, res, next) => {
  ...
  const getActiveRoom = {
    text: "SELECT rooms._id as id, rooms.name, rooms.admin_id as admin FROM users INNER JOIN rooms ON users.active_room = rooms._id WHERE users._id = $1",
    values: [userId]
  };
...
roomsController.joinRoom = async (req, res, next) => {
  ...
  try {
    ...
    const joinRoom = {
      text: "INSERT INTO rooms_users (user_id, room_id, banned) VALUES ($1, $2, false)",
